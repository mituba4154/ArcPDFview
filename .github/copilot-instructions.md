...
| # | タスク | 状態 | 備考 |
|---|--------|------|------|
| 0-1 | .NET 8 / Avalonia 11 環境確認 | ⬜ | |
| 0-2 | ソリューション構造の作成 | ⬜ | |
| 0-3 | NuGet パッケージ追加 | ⬜ | PDFiumのライセンス確認が必要 |
| 0-4 | 空ウィンドウ起動確認 | ⬜ | |
| 0-5 | カラースキーム定義 Colors.axaml | ⬜ | wireframe の CSS 変数をそのまま移植 |
| 0-6 | copilot-instructions.md 配置 | ⬜ | このファイル |
| 0-7 | GitHub Actions 設定 | ⬜ | Win + Linux のマトリクスビルド |
| 0-8 | README.md 作成 | ⬜ | |

**Phase 0 完了チェックリスト:**
- [ ] `dotnet build` がエラーなく通る
- [ ] Avalonia ウィンドウが Windows / Linux で起動する
- [ ] GitHub Actions 初回ビルドが成功する

**⚠️ Phase 0 実装時の注意点:**

```
PDFium の NuGet パッケージについて:
  - 第一候補: Sdcb.PdfiumBuild (MIT ライセンス)
  - 代替候補: PdfiumViewer.Native
  - 注意: PDFium.NET.SDK は商用利用に制限がある場合あり
